...
function criarCartaoSobre(vaga: Vaga): string {
...
export function inicializarDetalhesVaga(
  vagasUlElement: HTMLElement,
  vagasSobreElement: HTMLElement,
  getVagasCarregadas: () => Vaga[]
): void {
  ...
  const handleMouseOver = (e: Event) => {
    ...
    const vagaLi = target.closest('.vagaLI') as HTMLElement | null;
  ...
  vagasUlElement.addEventListener('mouseover', debounce(handleMouseOver, 150) as EventListener);
}
